...
try {
  ...
  nutjsAvailable = true;
  console.error("✅ nutjs loaded successfully");
} catch (error: any) {
  console.error("⚠️ nutjs not fully available:", error.message);
  console.error(
...
try {
  ...
  console.error("⚠️ macOS permissions module not available:", error.message);
}

// Suppress FastMCP console output and redirect to stderr
const originalConsoleLog = console.log;
const originalConsoleError = console.error;
const originalConsoleWarn = console.warn;

console.log = (...args: any[]) => {
  // Redirect all logs to stderr to avoid breaking MCP stdio protocol
  // MCP requires ONLY JSON-RPC messages on stdout
  if (args[0] && typeof args[0] === "string" && /^[🚀📋🔐⚠️✅]/.test(args[0])) {
    originalConsoleError(...args); // Use stderr instead of stdout
  }
};
...
server.addTool({
  name: "keyboard_type",
  description:
    "Simulate typing text or pressing key combinations. Provide either 'text' to type literal text, OR 'keys' as comma-separated key names for key combinations (not both).",
  parameters: z.union([
    z.object({
    ...
    z.object({
      keys: z
        ...
        .min(1)
        .describe(
          "Comma-separated key names to press simultaneously (e.g. 'LeftControl,C')"
        ),
    }),
  ]),
